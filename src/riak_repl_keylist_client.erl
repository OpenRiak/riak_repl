...
wait_for_fullsync(Command, State)
    ...
    Partitions = case State#state.partitions of
        [] ->
            case app_helper:get_env(riak_repl_progress, 
                ...
                Progress ->
                    ?LOG_INFO("Full-sync with site ~p; resuming failed fullsync at ~p",
    ...
    ?LOG_INFO("Full-sync with site ~p starting; ~p partitions.",
...
request_partition(continue, #state{partitions=[P|T], work_dir=WorkDir, socket=Socket} = State) ->
    ...
    case riak_repl_util:maybe_get_vnode_lock(P) of
        ok ->
            ?LOG_INFO("Full-sync with site ~p; starting fullsync for ~p",
...
request_partition({kl_exchange, P},  State) ->
    ?LOG_WARNING("Stale kl_exchange message received for ~p, ignoring",
        [P]),
...
request_partition({skip_partition, Partition}, #state{partition=Partition,
        kl_pid=Pid} = State) ->
    ?LOG_WARNING("Full-sync with site ~p; skipping partition ~p as requested by server",
...
request_partition({skip_partition, Partition}, State) ->
    ?LOG_WARNING("Full-sync with site ~p; asked to skip partition ~p, but current partition is ~p",
...
send_keylist(continue, #state{kl_fh=FH0,transport=Transport,socket=Socket,kl_counter=Count} = State) ->
    FH = case FH0 of
        undefined ->
            ?LOG_INFO("Full-sync for ~p; sending keylist for ~p",
                [State#state.sitename, State#state.partition]),
    ...
    case file:read(FH, ?MERKLE_CHUNKSZ) of
        ...
        eof ->
            ...
            ?LOG_INFO("Full-sync with site ~p; sent keylist for ~p (sent in ~p secs)",
                [State#state.sitename, State#state.partition,
                    riak_repl_util:elapsed_secs(State#state.stage_start)]),
            ?LOG_INFO("Full-sync with site ~p; exchanging differences for ~p",
                [State#state.sitename, State#state.partition]),
...
wait_ack(diff_done, State) ->
    ?LOG_INFO("Full-sync with site ~p; differences exchanged for ~p (done in ~p secs)",
        [State#state.sitename, State#state.partition,
            riak_repl_util:elapsed_secs(State#state.stage_start)]),
    ?LOG_INFO("Full-sync with site ~p; full-sync for partition ~p complete (done in ~p secs)",
        [State#state.sitename, State#state.partition,
...
log_stop(Command, State) ->
    ?LOG_INFO("Full-sync for site ~p ~s at partition ~p (after ~p secs)",
